...
const stages = [
  ...
  },
  {
    ...
    outputs: [
      {
        label: "Strategy artifact covering:",
        children: [
          "Problem(s) to solve",
          "Hypothesis",
      ...
      {
        label: "Example artifacts:",
        inline: "PRD, PRFAQ, brief, journey map, storyboard, video, etc.",
      },
    ],
    detail:
      "Synthesize research into a cohesive strategy artifact focusing on key problem(s) to solve. Multiple iterative rounds with Claude refining the narrative and output. Manual refinement on positioning, recommendations, and overall story. I use the 80/20 framework: 80% completed with AI, reserving the final 20% for human taste and judgment to be applied.",
  },
  {
    id: 3,
  ...
  },
  {
    ...
    detail:
      "Generate final artifacts (typically PRD or similar) that covers the most up-to-date prototype version. Present this and the functional prototype to users and/or stakeholders for final round of feedback. This is essentially a 'last pass' before looping in the broader PDE team to build out for real. Lead with async communication vehicle and provide the option for real-time conversation to dive deeper.",
  },
];
...
function OutputList({ outputs, phaseColor }) {
  return outputs.map((output, j) => {
      ...
      return (
        <div
          ...
          style={{
            ...
            gap: 8,
            padding: "5px 0",
            borderBottom: j < outputs.length - 1 ? `1px solid ${BORDER}` : "none",
          }}
        >
          <span style={{ color: phaseColor, fontSize: 7, flexShrink: 0 }}>●</span>
    ...
    if (output.inline) {
      return (
        <div key={j} style={{ padding: "5px 0", borderBottom: j < outputs.length - 1 ? `1px solid ${BORDER}` : "none" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            <span style={{ color: phaseColor, fontSize: 7, flexShrink: 0 }}>●</span>
            <span style={{ fontSize: 12 }}>
              <span style={{ color: "#ccc", fontWeight: 600 }}>{output.label}</span>{" "}
              <span style={{ color: "#999", fontWeight: 400 }}>{output.inline}</span>
            </span>
          </div>
        </div>
      );
    ...
    return (
      <div key={j} style={{ padding: "5px 0", borderBottom: j < outputs.length - 1 ? `1px solid ${BORDER}` : "none" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
          <span style={{ color: phaseColor, fontSize: 7, flexShrink: 0 }}>●</span>
          <span style={{ fontSize: 12, color: "#ccc", fontWeight: 600 }}>{output.label}</span>
        </div>
        {output.children && (
          <div style={{
            paddingLeft: 20,
            ...
            {output.children.map((child, k) => (
              <div key={k} style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "2px 0" }}>
                <span style={{ color: GRAY, fontSize: 6, flexShrink: 0 }}>○</span>
                <span style={{ fontSize: 11, color: "#aaa" }}>{child}</span>
              </div>
            ))}
          </div>
        )}
    ...
    >
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto 32px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          <span style={{ color: ACCENT, fontWeight: 900, fontSize: 14, letterSpacing: 3 }}>
            How I Build |{" "}
            <a
              href="https://www.linkedin.com/in/contact-ian-metz"
              target="_blank"
              ...
              onMouseEnter={(e) => (e.currentTarget.style.color = "#E8C9A4")}
              onMouseLeave={(e) => (e.currentTarget.style.color = "#D4AD82")}
            >
              Ian Metz
            </a>
          </span>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 32 }}>
          {[
            ...
            { label: "Person", value: "1", accent: "#EC4899" },
          ].map((stat) => (
            <div
              key={stat.label}
              style={{
            ...
            return (
              ...
              >
                <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: stage.outputs.some((o) => typeof o !== "string") ? "0 0 400px" : "0 0 280px" }}>
                    <div
                      style={{ fontSize: 10, color: GRAY, letterSpacing: 2, fontWeight: 600, marginBottom: 8 }}
                    >
                      OUTPUTS
                    </div>
                    <OutputList outputs={stage.outputs} phaseColor={phase.color} />
                  </div>
                </div>
              </div>
            );
        ...
        <div style={{ marginTop: 0 }}>
          {/* Down arrow from pipeline to PDE */}
          <div style={{ textAlign: "center", padding: "6px 0", color: GRAY, fontSize: 18 }}>↓</div>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "#7CB8D8", fontStyle: "italic", lineHeight: 1.5, marginBottom: 8 }}>
                Note: the broader PDE team can be looped in at any time upstream — this is the latest point where
                they should be added, depending on what is being built.
              </div>
              <div style={{ fontSize: 12, color: "#999", lineHeight: 1.6 }}>
                Once prototyping validates the direction, the full Product–Design–Engineering trifecta engages and
                defines the feasibility, usability, and viability of solution. Suite of artifacts are shared, sometimes
                including actual code files. Further documentation generated as needed to support build of production
                infrastructure, drive design-system adherence, and expand edge case coverage.
              </div>
            </div>
          </div>

          {/* Down arrow from PDE to future phases */}
          <div style={{ textAlign: "center", padding: "6px 0", color: GRAY, fontSize: 18 }}>↓</div>

          {/* Future phases row */}
          <div style={{ display: "flex", alignItems: "stretch", gap: 0 }}>
            {futurePhases.map((fp, i) => (
              <div key={fp.id} style={{ display: "flex", alignItems: "stretch", flex: "1 1 0" }}>
                <div
                  style={{
                    flex: 1,
                    padding: "14px 16px",
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "#bbb", lineHeight: 1.7 }}>
              ...
              completed — research, strategy, narratives, prototype brief, reference images, etc. — it gets fed back
              into the project context. Every subsequent stage has richer context, so the build{" "}
